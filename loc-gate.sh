...
#     verbatim for easy re-sync (the same crates are skipped by ci.sh's FORKS
#     list and rustfmt.toml's `ignore` — keep all three in sync); and
...
# list in ci.sh and the `ignore` list in rustfmt.toml.
EXCLUDED_PREFIXES=(
  "hygg-cff-parser/"
  "hygg-pdf-extract/"
)
