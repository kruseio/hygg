...
//! 2. **The default sync-server URL** from `hygg-gui/.env`, so a self-host /
//!    LAN build can point at its own server without hardcoding (or committing)
//!    an address. Unset → the SaaS default. Mirrors `hygg-pwa/build.rs`. An
...
/// Bake `HYGG_GUI_SERVER_URL` from `hygg-gui/.env` (unless already set in the
