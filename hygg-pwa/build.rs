...
//! 2. **The default server URL** from `hygg-pwa/.env`. A local / self-host dev
//!    build can point at a LAN server by setting `HYGG_PWA_SERVER_URL` in
//!    `hygg-pwa/.env` (gitignored) — this emits it as a compile-time env var
//!    that `Settings::default` reads via `option_env!`, so no address is
//!    hardcoded (or committed) in the source. Unset → the SaaS default. An
//!    explicit environment variable takes precedence over the `.env` file.
...
/// Bake `HYGG_PWA_SERVER_URL` from `hygg-pwa/.env` (unless already set in the
/// environment) so `Settings::default` can read it via `option_env!`.
fn emit_server_url() {
