//! Compile-time build provenance, surfaced by the About / Credits pages: the
//! crate version plus the git commit this bundle was built from (short + full
//! sha and its date), baked by `build.rs`. A build without git (rare — this
//! crate is never published) degrades gracefully to "unknown" / empty rather
