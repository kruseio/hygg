...
// The paths below go through `packages/hygg-server/docs/pages/`, which holds a
// symlink per page pointing back out at the repository's `docs/pages/`. The
// indirection is what makes this crate publishable: `cargo package` copies only
// files at or under the package root, so the repository-root `docs/pages/*.md`
// these once named directly were simply absent from the tarball and every
// `cargo publish -p hygg-server` died in the verification build — which is why
// the crate sat at 0.1.15 while the rest of the workspace moved on. Cargo
// resolves each symlink and writes the *content* into the tarball as a regular
// file, so the published crate carries real markdown while the repository keeps
// one copy of it, at the root, where README.md and docs/README.md link to it.
//
// The symlinks are relative, so their depth is load-bearing: each one climbs
// four levels (packages/hygg-server/docs/pages -> repository root). Moving this
// crate again means re-pointing all five, and a stale hop count dangles them —
// which `cargo build -p hygg-server` catches immediately, as these include_str!
// paths then resolve to nothing.
//
