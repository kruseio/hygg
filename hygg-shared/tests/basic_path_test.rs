...
fn test_basic_path_formats() {
  ...
  File::create("Nærri\\ lýsing\\ av\\ gjalding_flyting\\ \\(1\\).csv")
    .unwrap()
    .write_all(b"data")
    .unwrap();
  ...
  if cfg!(windows) {
    assert!(
      result.is_ok(),
      "Backslash should work on Windows: {:?}",
      result.err()
    );
  } else {
    // On Unix, backslash is literal character, should fail because no file with
    // that name exists
    assert!(
      result.is_err(),
      "Backslash should fail on Unix (literal character)"
    );
  }

  // Test 3: Path with spaces and special chars (unescaped - shell escaping is
  // handled before our function)
  let result =
    normalize_file_path("Nærri\\ lýsing\\ av\\ gjalding_flyting\\ \\(1\\).csv");
