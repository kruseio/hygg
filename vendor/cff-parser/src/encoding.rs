...
impl Encoding<'_> {
    ...
    pub fn get_table(&self) -> Vec<u8> {
        match self.kind {
            // hygg patch: upstream `get_sid` (see comment around line 117)
            // already treats Expert as Standard ("we allow only 8bit
            // codepoints"), but `get_table` panicked instead. Mirror the
            // Standard arm so Adobe-generated PDFs that use Expert-encoded
            // CFF fonts (e.g. pdfreference1.7old.pdf) parse without crashing.
