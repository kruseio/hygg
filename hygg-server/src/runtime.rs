...
/// workspace root, from `hygg-server/.env`. Explicit environment variables keep
/// priority because dotenvy does not overwrite existing values.
pub fn load_dotenv() {
...
fn dotenv_fallback_paths() -> Vec<PathBuf> {
  ...
  if let Ok(cwd) = std::env::current_dir() {
    paths.push(cwd.join("hygg-server").join(".env"));
  }
